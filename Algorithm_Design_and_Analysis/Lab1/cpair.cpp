...
int main()
{
    ...
    for (int i = 0; i <= row_idx - 1; i++)
        cout << points[i].ENODEBID << endl;
    for (int i = 0; i <= row_idx - 1; i++)
        for (int j = i + 1; j <= row_idx - 1; j++)
            if (points[i].ENODEBID == points[j].ENODEBID)
                cout << "Duplicate ENODEBID: " << points[i].ENODEBID << endl;
    ...
    cout << "The distance is: " << d << " km" << endl;
}
